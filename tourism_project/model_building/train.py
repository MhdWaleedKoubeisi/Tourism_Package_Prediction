...
os.environ["MLFLOW_SUPPRESS_PRINTING_URL_TO_STDOUT"] = "true"   # keeps the Actions log short (no "View run" line per trial)

# The workflow starts a local MLflow server on this address before running this script
...
# Define hyperparameter grid (96 combinations x 5-fold CV)
param_grid = {
    'xgbclassifier__n_estimators': [150, 250],        # Number of boosting trees
    'xgbclassifier__max_depth': [4, 6, 8],            # Maximum depth of each tree
    'xgbclassifier__colsample_bytree': [0.6, 0.8],    # Fraction of features sampled per tree
    'xgbclassifier__colsample_bylevel': [0.6, 0.8],   # Fraction of features sampled per tree level
    'xgbclassifier__learning_rate': [0.05, 0.1],      # Step size of boosting
    'xgbclassifier__reg_lambda': [1, 10],             # L2 regularization strength
}
# Model pipeline
...
with mlflow.start_run():
    # Hyperparameter tuning with GridSearchCV (F1 is used because the classes are imbalanced)
    ...
    # Classification threshold: chosen in the development notebook from out-of-fold predictions on the
    # training data (highest recall while F1 stays within 0.005 of its peak). Keep in sync with app.py.
    ...
    print(f"\nTEST classification report (threshold = {classification_threshold})")
